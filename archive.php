<?php
/**
 * Archivo del blog — "Journal" style:
 * - El post más reciente (o el primero de la página actual) va full-bleed,
 *   como un mini-hero, no como una tarjeta más.
 * - El resto en grid de tarjetas (mismo componente que usa el bloque de home).
 * - Filtro de categorías con pills nativas (link normal, sin JS/AJAX).
 */
get_header();
...
<header class="blog-archive__head container">
	...
	<?php if ( $categories ) : ?>
		<nav class="blog-archive__filters" aria-label="Filtrar por categoría">
			<a class="blog-archive__pill <?php echo ! $current_cat ? 'is-active' : ''; ?>" href="<?php echo esc_url( get_post_type_archive_link( 'post' ) ?: home_url( '/blog/' ) ); ?>">Todos</a>
...
<?php if ( have_posts() ) : ?>
	...
	<div class="container">
		...
		while ( have_posts() ) :
			...
			// El primer post de la primera página (no en páginas 2, 3...) va destacado.
			if ( $post_index === 1 && $is_first_page ) :
				...
				<?php
				// El grid arranca aquí; se cierra después del while.
				continue;
			endif;
			?>
